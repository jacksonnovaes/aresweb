...
function Detail({ label, children }: { label: string; children: React.ReactNode }) {
  return (
    <Box>
      <Typography variant="caption" color="#667085" sx={{ textTransform: "uppercase", letterSpacing: ".06em", fontWeight: 700 }}>{label}</Typography>
  ...
  return (
    <Dialog className="service-order-print-dialog" open={open} onClose={onClose} fullWidth maxWidth="md">
      <DialogContent sx={{ p: { xs: 1.5, sm: 3 }, bgcolor: "#EEF1F5" }}>
        {loading && <Stack alignItems="center" justifyContent="center" minHeight={360} spacing={2}><CircularProgress /><Typography color="#667085">Preparando a ordem para impressão...</Typography></Stack>}
        {!loading && error && <Box py={4}><ErrorAlert message={error} onRetry={onRetry} /></Box>}
        {!loading && document && (
          <Box className="service-order-print" sx={{ bgcolor: "white", color: "#172033", colorScheme: "light", p: { xs: 2.5, sm: 5 }, mx: "auto", maxWidth: 850, minHeight: 720, boxShadow: "0 12px 34px rgba(16,24,40,.12)" }}>
            <Box component="header" className="service-order-print-header" sx={{ borderBottom: `4px solid ${primaryColor}`, pb: 2.5, mb: 3 }}>
              <Stack direction="row" justifyContent="space-between" alignItems="flex-start" gap={3}>
                <Stack direction="row" spacing={2} alignItems="center">
                  ...
                  <Box><Typography variant="h6" fontWeight={850}>{document.company.tradeName}</Typography><Typography variant="body2" color="#667085">Razão social: {document.company.legalName}</Typography>{document.company.document && <Typography variant="body2" color="#667085">CPF/CNPJ: {maskDocument(document.company.document)}</Typography>}</Box>
                </Stack>
                <Box textAlign="right"><Typography variant="overline" color="#667085" fontWeight={800}>{delivery ? "Termo de entrega e garantia" : "Orçamento / ordem de serviço"}</Typography><Typography variant="h5" fontWeight={900} sx={{ color: primaryColor }}>#{document.order.id.slice(0, 8).toUpperCase()}</Typography><Typography variant="caption" color="#667085">Emitido em {formatDateTime(new Date().toISOString())}</Typography></Box>
              </Stack>
            </Box>

              <Stack direction={{xs: "column", sm: "row"}} gap={3} justifyContent="space-between">
                  <Box flex={1}><Typography variant="overline" fontWeight={800}
                                            color="#667085">Cliente</Typography>
                      <Typography variant="h6"fontWeight={800}>{document.customer.name}</Typography><Typography
                      variant="body2"
                      color="#667085">{[document.customer.email, document.customer.phone].filter(Boolean).join(" • ") || "Contato não informado"}</Typography>{document.customer.document &&
                      <Typography variant="body2" color="#667085">{maskDocument(document.customer.document)}</Typography>}{document.customer.address &&
                      <Typography variant="body2" color="#667085" sx={{whiteSpace: "pre-wrap"}}>{document.customer.address}</Typography>}</Box>
                  {document.asset &&
                      <Box flex={1}><Typography variant="overline" fontWeight={800} color="#667085">Ativo em manutenção</Typography>
                          <Typography variant="h6" fontWeight={800}>{document.asset.name}</Typography>
                          <Typography variant="body2" color="#667085">{document.asset.typeName}{assetDescription ? ` • ${assetDescription}` : ""}
                          </Typography>{document.asset.serialNumber &&
                          <Typography variant="body2" color="#667085">Série: {document.asset.serialNumber}</Typography>}</Box>
                  }
              </Stack>
            ...
            <Typography variant="body2" color="#667085" mt={0.75} sx={{ whiteSpace: "pre-wrap" }}>{document.order.description || "Sem descrição adicional."}</Typography>
            ...
            <Table size="small" sx={{
              ...
              "& .MuiTableCell-root": { color: "#172033", borderColor: "#E5E9F2" },
              "& .MuiTableHead-root .MuiTableCell-root": { color: "#667085" },
            }}>
            ...
            {delivery && <Box sx={{ mt: 4, p: 2.5, border: `1px solid ${primaryColor}40`, borderLeft: `5px solid ${primaryColor}`, borderRadius: 2, breakInside: "avoid" }}>
              ...
              {delivery.warrantyTerms && <Box mt={2}><Typography variant="caption" color="#667085" fontWeight={750}>CONDIÇÕES DA GARANTIA</Typography><Typography variant="body2" mt={0.4} sx={{ whiteSpace: "pre-wrap" }}>{delivery.warrantyTerms}</Typography></Box>}
              {delivery.notes && <Box mt={1.5}><Typography variant="caption" color="#667085" fontWeight={750}>OBSERVAÇÕES DA ENTREGA</Typography><Typography variant="body2" mt={0.4} sx={{ whiteSpace: "pre-wrap" }}>{delivery.notes}</Typography></Box>}
            </Box>}
