...
const ACCESS_COOKIE = "ares_customer_access_token";
const REFRESH_COOKIE = "ares_customer_refresh_token";

type RouteContext = { params: Promise<{ path: string[] }> };

const allowedPaths = [
  /^auth\/(login|refresh|logout|me)$/,
  /^service-orders(?:\/[0-9a-f-]+)?$/i,
];

function upstreamPath(path: string) {
  if (path === "auth/login") return "customer/auth/login";
  if (path.startsWith("service-orders")) return `customer/${path}`;
  return path;
}
...
async function upstream(
  ...
  const target = new URL(`${API_URL}/api/v1/${upstreamPath(path)}`);
...
async function proxyHandler(request: NextRequest, context: RouteContext) {
  ...
  if (contentLength > 64 * 1024) {
    return NextResponse.json({ detail: "A requisição excede o limite permitido." }, { status: 413 });
  }
  ...
  if (path === "auth/logout") {
    ...
    return clearCustomerCookies(await copyResponse(response));
  }
  ...
  if (path === "auth/login") {
    return authResponse(await upstream(request, path, undefined, originalBody));
  }
