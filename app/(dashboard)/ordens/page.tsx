...
import type { Asset, CatalogService, CompanySettings, Customer, ManagedUser, ServiceOrder, ServiceOrderDocument, ServiceOrderEmailResult, ServiceOrderPriority, ServiceOrderStatus, ServiceOrderStatusDefinition } from "@/lib/types";
...
export default function OrdersPage() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      setCompanySettings(settingsData); setStatuses(statusData);
  ...
  async function loadPrintDocument(order: ServiceOrder) {
    setPrintLoading(true); setPrintError(""); setPrintDocument(null);
    try { setPrintDocument(await apiRequest<ServiceOrderDocument>(`/service-orders/${order.id}/document`)); }
  ...
  async function sendEmail(event: FormEvent) {
    ...
    } catch (err) { setEmailError(errorMessage(err)); }
  ...
  async function saveQuote() {
    ...
    try {
      ...
      setQuoteOrder(null);
    } catch (err) { setQuoteError(errorMessage(err)); }
    ...
    <>
      ...
      <Card>
        <Stack direction={{ xs: "column", md: "row" }} gap={2} sx={{ p: 2.5, borderBottom: "1px solid", borderColor: "divider" }}>
          ...
          <FormControl sx={{ minWidth: 220 }}><InputLabel>Status</InputLabel><Select label="Status" value={statusFilter} onChange={(e) => setStatusFilter(e.target.value)}><MenuItem value="all">Todos os status</MenuItem>{statusFilterOptions.map((status) => <MenuItem value={status.code} key={status.code}>{status.name}</MenuItem>)}</Select></FormControl>
          <Typography variant="body2" color="text.secondary" alignSelf="center" sx={{ ml: { md: "auto" } }}>{filtered.length} ordens</Typography>
        </Stack>
        <TableContainer><Table>
          <TableHead><TableRow><TableCell>Ordem</TableCell><TableCell>Cliente / atendimento</TableCell><TableCell>Prioridade</TableCell><TableCell>Status</TableCell><TableCell>Prazo</TableCell><TableCell align="right">Valor</TableCell>{can("SERVICE_ORDER_UPDATE") && <TableCell align="right">Próxima etapa</TableCell>}<TableCell align="right">Ações</TableCell></TableRow></TableHead>
          <TableBody>
            {loading && <TableLoading colSpan={can("SERVICE_ORDER_UPDATE") ? 8 : 7} />}
            {!loading && filtered.length === 0 && <TableEmpty colSpan={can("SERVICE_ORDER_UPDATE") ? 8 : 7} message="Nenhuma ordem de serviço encontrada." />}
            {filtered.map((order) => <TableRow key={order.id} hover>
              ...
              <TableCell><Typography variant="body2">{customerMap.get(order.customerId) ?? "Cliente"}</Typography><Typography variant="caption" color="text.secondary">{order.assetId ? assetMap.get(order.assetId) ?? "Ativo" : "Serviço sem ativo"}</Typography></TableCell>
              <TableCell><StatusChip value={order.priority} /></TableCell><TableCell><StatusChip value={order.status} label={statusMap.get(order.status)} /></TableCell><TableCell>{formatDate(order.dueAt)}</TableCell><TableCell align="right" sx={{ fontWeight: 700 }}>{formatMoney(order.finalValue ?? order.estimatedValue)}</TableCell>
              {can("SERVICE_ORDER_UPDATE") && <TableCell align="right"><Button size="small" endIcon={<ArrowForwardRoundedIcon />} onClick={() => startStatus(order)} disabled={!statuses.some((status) => status.code !== order.status)}>Alterar</Button></TableCell>}
              <TableCell align="right"><Stack direction="row" justifyContent="flex-end" spacing={0.25}>{can("SERVICE_ORDER_UPDATE") && <Tooltip title="Editar orçamento"><IconButton size="small" aria-label={`Editar orçamento da ordem ${order.title}`} onClick={() => startQuote(order)}><RequestQuoteOutlinedIcon fontSize="small" /></IconButton></Tooltip>}<Tooltip title="Imprimir ordem"><IconButton size="small" aria-label={`Imprimir ordem ${order.title}`} onClick={() => startPrint(order)}><PrintOutlinedIcon fontSize="small" /></IconButton></Tooltip>{can("SERVICE_ORDER_UPDATE") && <Tooltip title="Enviar por e-mail (simulação)"><IconButton size="small" aria-label={`Enviar ordem ${order.title} por e-mail`} onClick={() => startEmail(order)}><EmailOutlinedIcon fontSize="small" /></IconButton></Tooltip>}</Stack></TableCell>
            </TableRow>)}
          </TableBody>
      ...
      <Dialog open={Boolean(statusOrder)} onClose={() => !saving && setStatusOrder(null)} fullWidth maxWidth="xs"><DialogTitle>Alterar status da ordem<Typography variant="body2" color="text.secondary" mt={0.5}>{statusOrder?.title}</Typography></DialogTitle><DialogContent dividers><Stack spacing={2.25}>
        {formError && <Alert severity="error">{formError}</Alert>}
        <Box sx={{ p: 2, borderRadius: 2.5, bgcolor: "#F8FAFC" }}><Typography variant="caption" color="text.secondary">Status atual</Typography><Box mt={0.75}>{statusOrder && <StatusChip value={statusOrder.status} label={statusMap.get(statusOrder.status)} />}</Box></Box>
      ...
      <Dialog open={Boolean(emailOrder)} onClose={() => !emailSending && setEmailOrder(null)} fullWidth maxWidth="sm"><Box component="form" onSubmit={sendEmail}><DialogTitle>Enviar ordem por e-mail<Typography variant="body2" color="text.secondary" mt={0.5}>{emailOrder?.title}</Typography></DialogTitle><DialogContent dividers><Stack spacing={2.25}>
        ...
        <TextField label="Destinatário" type="email" value={recipient} onChange={(event) => setRecipient(event.target.value)} required fullWidth disabled={emailSending || Boolean(emailResult)} autoFocus />
        {emailResult && <Box sx={{ border: "1px solid", borderColor: "divider", borderRadius: 2, overflow: "hidden" }}><Box sx={{ p: 1.75, bgcolor: "#F7F8FA", borderBottom: "1px solid", borderColor: "divider" }}><Typography variant="caption" color="text.secondary">Assunto</Typography><Typography variant="body2" fontWeight={750}>{emailResult.subject}</Typography></Box><Typography component="pre" variant="body2" sx={{ p: 2, m: 0, whiteSpace: "pre-wrap", fontFamily: "inherit", maxHeight: 300, overflow: "auto" }}>{emailResult.body}</Typography></Box>}
